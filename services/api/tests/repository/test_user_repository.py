...
from src.core.context import TenantContext, reset_tenant_context, set_tenant_context
...
async def test_create_and_lookup_by_email(db_session: AsyncSession) -> None:
    ...
    found = await repo.get_by_email_in_org(
        organization_id=org_id, email="a@example.com"
    )
...
async def test_get_by_id_is_tenant_scoped(db_session: AsyncSession) -> None:
    ...
    user_a = await repo.create(
        organization_id=org_a, email="u@a.com", password_hash="x", role="admin"
    )

    # With org_b as the active tenant, the org_a user must not be visible.
    token = set_tenant_context(
        TenantContext(user_id=str(uuid.uuid4()), organization_id=str(org_b), role="admin")
    )
    try:
    ...
    token = set_tenant_context(
        TenantContext(user_id=str(user_a.id), organization_id=str(org_a), role="admin")
    )
    try:
...
async def test_unscoped_lookup_crosses_tenants(db_session: AsyncSession) -> None:
    org_id = await _make_org(db_session, "org-unscoped")
    repo = UserRepository(db_session)
    user = await repo.create(
        organization_id=org_id, email="u@u.com", password_hash="x", role="admin"
    )
